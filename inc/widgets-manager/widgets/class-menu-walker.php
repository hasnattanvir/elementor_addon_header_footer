...
class Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		$submenu = $args->has_children ? ' sce-has-submenu' : '';

		if ( 0 === $depth ) {
			array_push( $classes, 'parent' );
		}
		$class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args, $depth ) );
		$class_names = ' class="' . esc_attr( $class_names ) . $submenu . ' sce-creative-menu"';
		...
		$atts = apply_filters( 'socoders_elements_nav_menu_attrs', $attributes );

		$item_output  = $args->has_children ? '<div class="sce-has-submenu-container">' : '';
		$item_output .= $args->before;
		$item_output .= '<a' . $atts;
		if ( 0 === $depth ) {
			$item_output .= ' class = "sce-menu-item"';
		} else {
			$item_output .= in_array( 'current-menu-item', $item->classes ) ? ' class = "sce-sub-menu-item sce-sub-menu-item-active"' : ' class = "hfe-sub-menu-item sce-sub-menu-item"';
		}

		$item_output .= '>';
		$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
		if ( $args->has_children ) {
			$item_output .= "<span class='sce-menu-toggle sub-arrow sce-menu-child-";
			$item_output .= $depth;
