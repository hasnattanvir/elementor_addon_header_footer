...
add_action( 'elementor/element/section/section_advanced/after_section_end', 'CurlWare_RegisterSectionControls', 10, 2 );
function CurlWare_RegisterSectionControls( $element, $sectionId ) {
	$element->start_controls_section('spria_sticky_sc',
		[
			'label'         => __('spria Sticky'),
			'tab' => \Elementor\Controls_Manager::TAB_STYLE,
		]
	);
	$element->add_control('spria_sticky',
		[
			'label'         => __( 'Enable'),
	...
	$element->add_control('spria_sticky_bg',
			[
				'type'      => \Elementor\Controls_Manager::COLOR,
				'label'     => esc_html__( 'Background', 'spria-core' ),
				'selectors' => [
					'{{WRAPPER}} .header-section.sticky' => 'background: {{VALUE}}',
				],
			]
		);
	$element->end_controls_section();
}
if (! function_exists( 'spria_pro_get_icons' ) ) {
    function spria_pro_get_icons() {
      return [
      'fa fa-500px'                               => '500px',
      ...
      'ri-arrow-right-line'                       => 'ri-arrow-right',
      'ri-arrow-right-up-line'                    => 'ri-arrow-right-up',
      'ri-download-line'                          => 'ri-download-line',
      'ri-check-line'                             => 'ri-check-line',
      'ri-play-circle-line'                       => 'ri-play-circle-line',
      'ri-play-fill'                              => 'play-fill',
      'ri-eye-line'                               => 'eye-line',
      'ri-download-cloud-line'                    => 'Download Line',
      ...
      'ri-checkbox-circle-line'                   => 'checkbox-circle-line'
      ];
}

}

if ( ! function_exists( 'spria_breadcrumb' ) ) {
 function spria_breadcrumb()
	{
		$breadcrumb_title = 'spria';
		...
		elseif (is_home()) :
			...
			if ($breadcrumb_on_off == '1') :
				$spria_page_title =  get_theme_mod('breadcrumb_title', '');
				if ($spria_page_title) {
					$blog_breadcrumb_content = $spria_page_title;
				}
		...
		elseif (is_single()) :
			if (get_post_type(get_the_ID()) == 'post') :
					// redux
					$blog_single_breadcrumb_content = esc_html__('Blog Details','spria');
				...
				// post type
				$breadcrumb_title = ucfirst(get_post_type()) . esc_html__(' Details', 'spria');
				$breadcrumb_class = get_post_type() . '-single-breadcrumb';
			endif;
		elseif (is_404()) :
			$breadcrumb_title = esc_html__('Error Page', 'spria');
			$breadcrumb_class = 'blog-breadcrumb';
		elseif (is_search()) :
			if (have_posts()) :
				$breadcrumb_title = esc_html__('Search Results for: ', 'spria') . get_search_query();
				$breadcrumb_class = 'blog-breadcrumb';
			else :
				$breadcrumb_title = esc_html__('Nothing Found', 'spria');
				$breadcrumb_class = 'blog-breadcrumb';
			endif;
		...
		if (is_page()) {
			...
			$spria_page_title      = '';
			if ($spria_page_title != '') {
				$breadcrumb_title = $spria_page_title;
			} else {
				$breadcrumb_title = get_the_title();
			}
		} else {
			$spria_show_breadcrumb = 'on';
		}

		$breadcrumb_class_with_header = 'breadcrumb-class-with-header-one';
		?>
		<?php
		if ( 'off' != $spria_show_breadcrumb ) :
		?>
			<?php
			if (isset($breadcrumb_title) && !empty($breadcrumb_title)) :
			?>
			<!--=========== breadcrumb Section Start =========-->
			<div class="sc-breadcrumb-style sc-pt-135 sc-pb-110 <?php echo esc_attr($breadcrumb_class . ' ' . $breadcrumb_active_class . ' ' . $breadcrumb_class_with_header); ?>">
